...
export const createEventSchema = z.object({
  body: z.object({
    ...
    event_type: z.string().min(1, 'Event type is required').default('party'),
    description: z.string().optional(),
    event_date: z.string().datetime('Invalid date format. Use ISO datetime string'),
    dj_artist: z.string().optional(),
    ticket_price: z.number().min(0, 'Ticket price must be positive').default(0).optional(),
    max_capacity: z.number().int().min(1, 'Max capacity must be at least 1').default(100),
